...
const LessonEditor = ({
  ...
  const renderContentModal = () => {
    console.log(
      "Rendering content modal for lesson type:",
      section,
      lesson,
      courseId
    );
    ...
    switch (lesson.type) {
      ...
      case "video":
      default: {
        let videoData = lesson.files?.[0];
        let fileId = lesson.files?.[0]?._id || lesson?._id;
        if (!videoData && courseData?.modules) {
          for (const mod of courseData.modules) {
            if (mod.lessons) {
              for (const l of mod.lessons) {
                if (l._id === lesson._id && l.files[0]) {
                  videoData = l.files?.[0];
                  fileId = l.files?.[0]._id;
                  console.log("Found video data in courseData:", videoData);
                }
              }
            }
          }
