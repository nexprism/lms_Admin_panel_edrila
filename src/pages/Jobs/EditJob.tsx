...
import { updateJob, selectJobLoading, selectJobError, selectJobs } from '../../store/slices/job';
...
const EditJob: React.FC = () => {
    ...
    if (id && jobs.length > 0) {
      ...
      if (jobToEdit) {
        setFormData({
          ...
          experienceLevel: jobToEdit.experienceLevel as 'beginner' | 'intermediate' | 'expert',
          estimatedDuration: {
            value: jobToEdit.estimatedDuration.value,
            unit: jobToEdit.estimatedDuration.unit as 'hours' | 'days' | 'weeks' | 'months'
          },
          mode: jobToEdit.mode as 'full-time' | 'part-time' | 'contract' | 'freelance' | 'internship',
          location: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const jobData = { ...formData, thumbnail: formData.thumbnail || undefined };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Edit Job</h2>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Budget Range</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                  Minimum Budget *
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Duration</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                  Duration Value *
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Required Skills</h3>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Job Image</h3>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                Thumbnail Image
              </label>
              ...
              {(existingImageUrl || formData.thumbnail) && (
                <div className="mt-2">
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                    {existingImageUrl ? 'Current image:' : 'New image preview:'}
                  </p>
                  <img
                    src={existingImageUrl || URL.createObjectURL(formData.thumbnail!)}
                    alt={existingImageUrl ? "Current job thumbnail" : "New job thumbnail preview"}
                    className="w-32 h-20 object-cover rounded-md border border-gray-300 dark:border-gray-600"
                    onError={(e) => {
                            console.log('Error loading image, setting fallback.',error);
