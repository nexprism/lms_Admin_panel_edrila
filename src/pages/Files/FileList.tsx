...
import { fetchFiles } from "../../store/slices/files";
...
const FileList: React.FC = () => {
  ...
  const { data, loading, error } = useAppSelector((state) => state.file);
  ...
  useEffect(() => {
    if (Array.isArray(data)) {
      ...
      let filtered = data;
      if (searchInput) {
        filtered = filtered.filter((f: File) => {
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= Math.ceil(filteredFiles.length / limit)) {
      setPage(newPage);
    }
  };
