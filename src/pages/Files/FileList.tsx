...
const VideoAnalyticsDashboard = () => {
  ...
  const { dashboard, loading, error } = useSelector((state: RootState) => state.analytics);
  ...
  if (loading) return (
    <div className="flex justify-center items-center min-h-screen bg-gray-50 text-blue-700">
      <div className="flex flex-col items-center">
        <svg className="animate-spin h-8 w-8 text-blue-700 mb-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
  ...
  const filteredVideos = dashboard.videoPerformance?.filter(video =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 p-4 sm:p-6 lg:p-8 font-sans">
      <h1 className="text-3xl sm:text-4xl font-extrabold mb-8 text-gray-900 drop-shadow-sm">Video Analytics Dashboard</h1>

      {/* Overview Section */}
      <section className="mb-10">
        <h2 className="text-xl sm:text-2xl font-bold mb-5 text-gray-800 flex items-center gap-2">
          <TrendingUp className="text-blue-600" size={24} /> Overview
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          {/* Total Views Card */}
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 hover:border-blue-500 transition-all duration-300 transform hover:scale-105">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-700">Total Views</h3>
              <Eye className="text-blue-600" size={28} />
            </div>
            <p className="text-4xl font-bold text-gray-900">{formatNumber(dashboard.overview?.totalViews || 0)}</p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 hover:border-blue-500 transition-all duration-300 transform hover:scale-105">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-700">Total Watch Time</h3>
              <Clock className="text-blue-600" size={28} />
            </div>
            <p className="text-4xl font-bold text-gray-900">{formatWatchTime(dashboard.overview?.totalWatchTime || 0)}</p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 hover:border-red-500 transition-all duration-300 transform hover:scale-105">
            <div className="flex items-center justify-between mb-4">
      ...
      <section className="mb-10 bg-white p-6 rounded-xl shadow-md border border-gray-200">
        ...
        <div className="flex items-center mb-4 bg-gray-50 rounded-lg border border-gray-300 focus-within:border-blue-500 transition-colors duration-200">
          ...
          <input
            ...
            className="w-full bg-transparent text-gray-700 px-3 py-2 rounded-lg focus:outline-none placeholder-gray-400"
          />
        </div>
        {filteredVideos.length > 0 ? (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Views</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Engagement</th>
                </tr>
              </thead>
          ...
          <p className="text-gray-500 text-center py-8">No videos found matching your search.</p>
        )}
      </section>
        ...
        {openUserEngagement && (
          <div className="overflow-x-auto rounded-lg border  max-h-[700px] overflow-scroll border-gray-200 mt-4">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Videos Watched</th>
            ...
            {dashboard.userEngagement?.length === 0 && (
              <p className="text-gray-500 text-center py-8">No user engagement data available.</p>
            )}
          </div>
        )}
      </section>
