...
import { CheckCircle, XCircle, Search, Filter, RotateCcw, Pencil, Trash2 } from "lucide-react";
...
const Banner: React.FC = () => {
    ...
    const filteredBanners = banners.filter((banner) => {
        const matchesSearch = !searchInput || banner.title.toLowerCase().includes(searchInput.toLowerCase());
        const matchesStatus = !statusFilter || (statusFilter === "active" && banner.isActive) || (statusFilter === "inactive" && !banner.isActive);
    ...
    return (
        <div>
            <PageMeta title="Banner List | LMS Admin" description="List of all banners" />
            <PageBreadcrumb pageTitle="Banner List" />
            <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] xl:px-10 xl:py-12">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white/90">Banners</h1>
                    <button className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors" onClick={() => window.location.href = "/banner/add"}>Add Banner</button>
                </div>
                {/* Search & Filter */}
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="bg-white divide-y divide-gray-100 dark:bg-gray-900 dark:divide-gray-800">
                            {filteredBanners.map((banner, idx) => (
                                <tr key={banner._id} className="hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
                                    ...
                                    <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{banner.title}</td>
                                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{banner.type}</td>
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{banner.priority}</td>
                                    <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{banner.createdAt ? new Date(banner.createdAt).toLocaleDateString() : "-"}</td>
                            ...
                            {filteredBanners.length === 0 && (
                                <tr>
                                    <td colSpan={8} className="text-center py-8 text-gray-400 dark:text-gray-500">No banners found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
